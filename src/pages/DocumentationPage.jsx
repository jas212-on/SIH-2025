...
import {
  ArrowLeft, BookOpen, Rocket, Network, Layers, Globe2, Code2, ShieldQuestion, ArrowRight,
} from 'lucide-react';
...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      'Every chat query goes through five steps: (1) language detection, so a question in Hindi or Malayalam is handled correctly; (2) a Cypher query against the Neo4j knowledge graph pulls structured relationships — state → district → year → metric; (3) the same query is embedded and matched against Pinecone for semantically similar assessment text; (4) both result sets are merged into a single context; (5) Gemini synthesizes that context into a natural-language, cited answer.',
      "This dual-pipeline design means Jalmitra doesn't hallucinate numbers the way a pure-LLM chatbot might — every figure it states traces back to either a graph relationship or a retrieved document, not the model's own memory.",
    ],
  },
  {
    id: 'data-sources',
  ...
  },
  {
    ...
    body: [
      ...
      'You choose your role from the sidebar in the Chat page at any time — it changes both the tone of chat answers and which quick-action shortcuts are surfaced.',
    ],
  },
  {
    id: 'api',
  ...
  },
  {
    ...
    body: [
      '"The map/chat says data is temporarily unavailable" — Neo4j Aura\'s free tier pauses after a period of inactivity; the backend degrades gracefully to semantic-only results when this happens rather than failing outright. It resumes within a few seconds of the next request.',
